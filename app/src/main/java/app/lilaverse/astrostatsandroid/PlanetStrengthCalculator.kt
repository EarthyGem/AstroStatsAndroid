...
class PlanetStrengthCalculator(
    ...
    fun getTotalPowerScoresForPlanetsCo(bodies: List<Coordinate>): Map<Coordinate, Double> {
        ...
        return (aspectScores.keys + houseScores.keys).associateWith { body ->
            (aspectScores[body] ?: 0.0) + (houseScores[body] ?: 0.0)
        }
    }
    ...
    private fun getHouseScore(body: Coordinate): Double {
        ...
        return power - ((body.longitude - cusp.value) * (variation / size))
    }
    ...
    fun allCelestialAspectScoresByAspect(bodies: List<Coordinate>): Map<CelestialAspect, Double> {
        ...
        aspects.forEach { aspect ->
            if (aspect.kind == Kind.Parallel) {
                val house1 = houseProvider(aspect.body1)
                val house2 = houseProvider(aspect.body2)
                val orb = getParallelOrbCategory(house1, house2, luminaryChecker(aspect.body1), luminaryChecker(aspect.body2))
                val actualOrb = aspect.orbDelta
                scoreDict[aspect] = (1 - abs(actualOrb)) * orb
            }
            // Non-parallel scoring can be added here
        }
        return scoreDict
    }
